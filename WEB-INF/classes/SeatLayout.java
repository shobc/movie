...
public class SeatLayout{
	...
	public static void main(String[] args) {
		String a =SeatLayout.seatingChart("1");
	...
	//座席を返すメソッド
	public static String seatingChart(String key) {
		//戻り値で使う変数　初期化
		...
		//予約済みの座席番号
		String seatReservationStr = "B1,B2,A8";
		//使用不可能な座席番号
...
// //空列の追加ー１２/１３ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーここからーーーーーー
// 		int retu = 2;
// 		int addRetu = 0;
// 		// addRetu = addRetu + seatMax[0] * 3 + 4 + (int)(cn.get(0)); 
// 		// stb.insert(addRetu,"\n");
// 		// addRetu = addRetu + seatMax[1] * 3 + 4 + (int)(cn.get(1));
// 		// stb.insert(addRetu,"\n"); 
// 		if(retu != 0){
// 			if(retu == 1){
// 				addRetu = addRetu + seatMax[retu-1] * 3 + 4 + (int)(cn.get(retu-1)); 
// 				stb.insert(addRetu,"\n");
// 			}
// 			if(retu == 2){
// 				addRetu = addRetu + seatMax[retu-2] * 3 + 4 + (int)(cn.get(retu-2)); 
// 				addRetu = addRetu + seatMax[retu-1] * 3 + 4 + (int)(cn.get(retu-1)); 
// 				stb.insert(addRetu,"\n");
// 			}
// 			if(retu == 3){
// 				addRetu = addRetu + seatMax[retu-3] * 3 + 4 + (int)(cn.get(retu-3)); 
// 				addRetu = addRetu + seatMax[retu-2] * 3 + 4 + (int)(cn.get(retu-2)); 
// 				addRetu = addRetu + seatMax[retu-1] * 3 + 4 + (int)(cn.get(retu-1)); 
// 				stb.insert(addRetu,"\n");
// 			}
// 		}
// //空列の追加ー１２/１３ーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーーここまでーーーーーー
	...
	private static List<Seat> getRow(int max, char prefix) {
		...
		for(int i = 1; i <= max; i++)
		rowData.add(seatLayout.new Seat(
